...
#   suppressMessages({
#     empty.mtofis.df <- CreateMotifCombs(top_motifs)
...
#   filter(motif1 %in% top_motifs & motif2 %in% top_motifs) %>%
#   ggasym::asymmetrise(motif1, motif2) %>%
#   dplyr::right_join(CreateMotifCombs(top_motifs)[, c(1,2)])
...
#     motifs_list <- ifelse(exclusive_motifs, DiscardSharedMotifs(motifs_list), motifs_list)
...
#     all <- CreateMotifCombs(motifs_top)
...
# plot.moti.pair.overlap.func <- function(pmet_split,
#                                         motifs_list,
#                                         counts = "p_adj",
#                                         by_cluster = FALSE) {
