...
bool cFimoFile::readFile(bool hasBinScore)
{
    ...
    std::cout << std::endl
              << "\t" << fimoHits.size() << " genes and " << numLines << " hits found" << std::endl;
    return true;
}
...
std::pair<std::string, double> cFimoFile::process(long k, long N, std::unordered_map<std::string, long> &promSizes)
{
    ...
    for (auto &hit : fimoHits)
    {
        ...
        for (std::vector<cMotifHit>::iterator m1 = hit.second.begin(); m1 < (hit.second.end()); m1++)
        {
            ...
            if (m1 == (hit.second.begin() + (k - 1)))
            { // pointing a kth hit, delete all later ones
    ...
    std::sort(binThresholds.begin(), binThresholds.end(), [](const std::pair<double, std::string> &a, const std::pair<double, std::string> &b)
    ...
    for (auto &hit : fimoHits)
    {
        ...
        auto binVal = std::find_if(binThresholds.begin(), binThresholds.end(), [&geneID](const std::pair<double, std::string> a)
                                    { return a.second == geneID; });
