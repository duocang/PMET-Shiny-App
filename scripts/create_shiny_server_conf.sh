...
# done
while true; do
    SHINY_PORT=$(( ( RANDOM % 60000 )  + 1025 ))  # 选择一个 1025 到 65535 之间的随机端口
    (echo > /dev/tcp/localhost/$SHINY_PORT) >/dev/null 2>&1
    result=$?
    if [ $result -ne 0 ]; then  # 如果返回非0值，表示端口未被占用
        echo "Selected free port: $SHINY_PORT"
        break
    fi
done
...
if [ "${SHINY_START}" == "yes" ]; then
    ...
    # Configuration file
    echo "${CONFIG}" > "conf/shiny-server.conf";
